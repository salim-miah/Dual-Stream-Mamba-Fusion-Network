...
architectures for both audio and visual streams. The model is trained on both
real and fake samples from all four dataset categories and evaluated on a
separate test split to ensure proper generalization.

GPU/Device Support:
- Automatic device selection (CUDA → MPS → CPU) or manual override via --device
- Mixed precision training (AMP) on CUDA via --amp (enabled by default on CUDA)
- Proper pin_memory and num_workers for better throughput on GPU
- Device-aware printing and deterministic configuration options
...
class Config:
    ...
    def __init__(self):
        ...
        self.max_videos_per_class = 2000
        ...
        # Device Configuration (resolved later)
        self.device = torch.device("cpu")
        self.use_amp = False      # Mixed precision flag (enabled on CUDA by default)
...
def process_visual_stream(video_path: str, config: Config) -> Optional[np.ndarray]:
    ...
    try:
        ...
        # Return None if we don't have enough valid frames
...
def process_audio_stream(video_path: str, config: Config) -> Optional[torch.Tensor]:
    """Process video to extract audio mel-spectrogram chunks."""
    try:
        ...
        try:
            ...
            # Load audio
            y, sr = librosa.load(tmpwav, sr=config.aud_sample_rate)
...
class VisualStreamTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Extract CNN features (features are frozen here for efficiency)
        with torch.no_grad():
...
class DeepfakeDataset(Dataset):
    """Dataset class for loading and processing deepfake detection data."""
    ...
    def __getitem__(self, idx: int) -> Tuple:
        ...
        try:
            visual_frames = process_visual_stream(video_path, self.config)
        ...
        try:
            audio_features = process_audio_stream(video_path, self.config)
...
# =============================================================================
# TRAINING AND EVALUATION FUNCTIONS (with AMP support)
# =============================================================================

def train_epoch(model: nn.Module, train_loader: DataLoader, optimizer: optim.Optimizer,
                criterion: nn.Module, device: torch.device, epoch: int, total_epochs: int,
                use_amp: bool, amp_dtype: torch.dtype) -> Tuple[float, float]:
    ...
    print(f"Epoch {epoch+1} - Train Loss: {avg_loss:.4f}, Time: {epoch_time:.2f}s, ETA: {eta_total/60:.2f}min")
...
def main():
    """Main execution function."""
    parser = argparse.ArgumentParser(description='Deepfake Detection using Audio-Visual Transformer Fusion (GPU-ready)')
    parser.add_argument('--data_dir', type=str, required=True,
                        help='Path to FakeAVCeleb_v1.2 dataset directory')
    ...
    parser.add_argument('--learning_rate', type=float, default=1e-4,
                        help='Learning rate')
    parser.add_argument('--max_videos_per_class', type=int, default=1000,
                        help='Maximum number of videos per class directory')
    ...
    parser.add_argument('--amp', action='store_true',
                        help='Enable AMP mixed precision (CUDA only). If not set, enables automatically on CUDA.')
    ...
    print("="*80)
    print("DUAL-STREAM AUDIO-VISUAL TRANSFORMER DEEPFAKE DETECTION (GPU-ready)")
    print("="*80)
    ...
    # Directory paths
    real_real_dir = config.data_dir / "RealVideo-RealAudio"
    fake_fake_dir = config.data_dir / "FakeVideo-FakeAudio"
    fake_real_dir = config.data_dir / "FakeVideo-RealAudio"
    real_fake_dir = config.data_dir / "RealVideo-FakeAudio"
    ...
    # Real videos (label = 0)
    real_files = collect_videos_recursive(real_real_dir, config.video_exts, config.max_videos_per_class)
    all_files.extend(real_files)
    all_labels.extend([0] * len(real_files))
    print(f"Real videos (RealVideo-RealAudio): {len(real_files)}")
    describe_collection("Real (sample)", real_files[:20])

    # Fake videos (label = 1) from all three fake directories
    fake_dirs = [fake_fake_dir, fake_real_dir, real_fake_dir]
    # Cap per directory to keep total under max_videos_per_class for fairness
    fake_files_per_dir = config.max_videos_per_class // 3 if config.max_videos_per_class else None

    total_fake_files = 0
    for fake_dir in fake_dirs:
        fake_files = collect_videos_recursive(fake_dir, config.video_exts, fake_files_per_dir)
        all_files.extend(fake_files)
        all_labels.extend([1] * len(fake_files))
        total_fake_files += len(fake_files)
        print(f"Fake videos ({fake_dir.name}): {len(fake_files)}")
    ...
    # Create datasets
    train_dataset = DeepfakeDataset(train_files, train_labels, config, transform)
    val_dataset = DeepfakeDataset(val_files, val_labels, config, transform)
    test_dataset = DeepfakeDataset(test_files, test_labels, config, transform)
    ...
    for epoch in range(config.epochs):
        ...
        train_total = 0
        with torch.no_grad():
            for visual_data, audio_data, labels in train_loader:
                try:
                    ...
                    with torch.autocast(device_type=config.device.type, dtype=config.amp_dtype, enabled=(config.use_amp and config.device.type == "cuda")):
                        logits = model(visual_data, audio_data)
                    predictions = torch.argmax(logits, dim=1)
                    train_correct += (predictions == labels).sum().item()
                    train_total += labels.size(0)
                except Exception as e:
                    print(f"Training accuracy calculation error: {e}")
                    continue
