...
"""
ABLATION STUDY - V1_HMF: Hierarchical Mamba Fusion

This script implements the "Hierarchical Mamba Fusion" architecture[cite: 7].
It uses a third Mamba block to explicitly model the temporal relationship
between the visual and audio streams[cite: 8].

Visual: MobileNetV3-Small (576) → Mamba (T=16) ↘
                                                Concat (T=16) → Fusion Mamba → MLP
...
class Config:
    def __init__(self):
        ...
        # [HIERARCHICAL FUSION CHANGE]: Set T_audio = T_visual for step-by-step fusion 
        self.aud_num_chunks = 16  # Was 5, now matches vis_num_frames
        self.aud_chunk_duration = 5.0 / 16.0 # Was 1.0. (Total duration 5s preserved: 16 * (5/16) = 5)
        self.aud_n_mels = 128
...
print(f"⚡ ARCHITECTURE: Hierarchical Mamba Fusion (T={config.vis_num_frames})")
...
def process_audio_stream(video_path: str, config: Config):
    try:
        ...
        for i in range(config.aud_num_chunks):
            ...
            # [HIERARCHICAL FUSION CHANGE]: Handle potential empty chunks if duration is very small
            if len(chunk) == 0:
...
# --- 5. HIERARCHICAL FUSION MODEL ARCHITECTURE --- [HIERARCHICAL FUSION CHANGE]
...
class VisualStream_MobileNetV3Small(nn.Module):
    ...
    def forward(self, x):
        ...
        # [HIERARCHICAL FUSION CHANGE]: Return the full sequence, not just the last state 
        # return temporal_out[:, -1, :] # Old way (B, D_model)
        return temporal_out # New way (B, T, D_model)
...
class HierarchicalFusionModel(nn.Module):
    """
    [HIERARCHICAL FUSION CHANGE]: This is the new fusion model.
    Implements Hierarchical Mamba Fusion[cite: 7].
    A third Mamba block models the temporal relationship between streams[cite: 8].
    """
    def __init__(self, config):
        super().__init__()
        ...
        # [HIERARCHICAL FUSION CHANGE]: Define the third "Fusion Mamba Block" 
        # Its input dimension is the sum of the two stream dimensions.
        fusion_mamba_d_model = config.vis_mamba_d_model + config.aud_mamba_d_model
        ...
        # [HIERARCHICAL FUSION CHANGE]: The fusion_head now processes the output
        # of the new fusion_mamba block.
    ...
    def forward(self, visual_input, audio_input):
        # 1. Get full hidden state sequences from both streams [cite: 10, 11]
        visual_seq = self.visual_stream(visual_input) # (B, T, D_vis)
        audio_seq = self.audio_stream(audio_input)   # (B, T, D_aud)

        # 2. Concatenate hidden states at each time step t [cite: 13]
        # Shape becomes (B, T, D_vis + D_aud)
        fused_seq = torch.cat((visual_seq, audio_seq), dim=2)

        # 3. Feed the new "fused sequence" into the third Mamba block 
        # Shape remains (B, T, D_vis + D_aud)
...
def main():
    ...
    print("\n" + "="*80 + "\nSTEP 4: BUILDING HIERARCHICAL FUSION MODEL\n" + "="*80) # [HIERARCHICAL FUSION CHANGE]

    model = HierarchicalFusionModel(config).to(config.device) # [HIERARCHICAL FUSION CHANGE]

    total_params, trainable_params = count_parameters(model)
    model_size_mb = get_model_size_mb(model)

    print(f"📊 HierarchicalFusionModel Statistics:") # [HIERARCHICAL FUSION CHANGE]
    print(f"   Total Parameters: {total_params:,}")
    print(f"   Trainable Parameters: {trainable_params:,}")
    print(f"   Model Size: {model_size_mb:.2f} MB")
    print(f"   Parameters (Millions): {total_params/1e6:.3f}M")
    
    # [HIERARCHICAL FUSION CHANGE]: Note the increase in parameters due to the new Fusion Mamba
    print(f"   (Note: Params > V1 due to the new Fusion Mamba block)")


    print("\n" + "="*80 + "\nSTEP 5: TRAINING HIERARCHICAL FUSION MODEL\n" + "="*80) # [HIERARCHICAL FUSION CHANGE]
    ...
    # [HIERARCHICAL FUSION CHANGE]: Updated model path
    model_path = os.path.join(config.model_save_dir, 'v1_hierarchical_mamba_best.pth')
    ...
    # Handle case where training stopped early
    ...
    print(f"\n" + "="*80)
    print(f"📈 V1_HMF: HIERARCHICAL FUSION (MOBILENETV3) - FINAL RESULTS") # [HIERARCHICAL FUSION CHANGE]
    print(f"="*80)
    ...
    plt.title('V1_HMF (Hierarchical Fusion): Training and Validation Loss') # [HIERARCHICAL FUSION CHANGE]
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
