...
"""
CROSS-DATASET EVALUATION: HMF_MultiDataset (AVLips+DeepSpeak+LipSyncTIMIT) → KoDF
Evaluates the Multi-Stream Hierarchical Fusion model trained on 3 datasets against KoDF.
Architecture: 3 Visual Streams (MobileNetV3) + 1 Audio Stream (MobileNetV3) → Fusion Mamba → MLP
"""
...
class Config:
    def __init__(self):
        ...
        self.model_path = "/home/affshafee/T2430421/pipelines/novel_fusion/HMF_multi_dataset/v1_hmf_multidataset_best.pth"
        self.output_dir = "/home/affshafee/T2430421/pipelines/HMF_md_to_KoDF"
        ...
        # All streams use MobileNetV3-Small with D=160
...
print(f"⚡ ARCHITECTURE: Multi-Stream HMF trained on 3 datasets (T={config.vis_num_frames})")
print(f"   Training: AVLips + DeepSpeak + LipSyncTIMIT")
print(f"   All streams: MobileNetV3 (576) + Mamba (D=160)")
...
class VisualStream_MobileNetV3Small(nn.Module):
    """MobileNetV3-Small for visual feature extraction."""
...
class HierarchicalFusionModel(nn.Module):
    """
    Multi-Stream Hierarchical Mamba Fusion Model.
    All streams use MobileNetV3 with D=160
    3 visual streams + 1 audio stream → Fusion Mamba → MLP
    """
    def __init__(self, config):
        super().__init__()
        # All 3 visual streams use MobileNetV3
        ...
        # Fusion Mamba Block - expects 4 * 160 = 640 dimensions
        fusion_mamba_d_model = 4 * config.vis_mamba_d_model  # 640
    ...
    def forward(self, visual_input_local, visual_input_context, visual_input_global, audio_input):
        ...
        local_seq = self.visual_stream_local(visual_input_local)      # (B, T, 160)
        ...
        # Concatenate all streams (B, T, 640)
...
def plot_confusion_matrix(cm, output_path):
    ...
    plt.title('Confusion Matrix: HMF_MultiDataset → KoDF')
...
def main():
    print("\n" + "="*80)
    print("CROSS-DATASET EVALUATION: HMF_MultiDataset → KoDF")
    print("="*80)
    ...
    # Step 3: Load model
    print("\n" + "="*80)
    print("LOADING MULTI-DATASET MODEL")
    print("="*80)
    ...
    print("="*80)
    print(f"\n🔥 Model: HMF_MultiDataset (4-Stream Hierarchical Fusion)")
    print(f"   Architecture: 4x MobileNetV3 (576) + Mamba (D=160)")
    print(f"🎯 Trained on: AVLips + DeepSpeak + LipSyncTIMIT")
    ...
    cm_path = os.path.join(config.output_dir, 'confusion_matrix_hmf_md_kodf.png')
    plot_confusion_matrix(metrics['confusion_matrix'], cm_path)

    # Step 8: Save metrics to file
    results_path = os.path.join(config.output_dir, 'hmf_md_evaluation_results.txt')
    with open(results_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("CROSS-DATASET EVALUATION: HMF_MultiDataset → KoDF\n")
        f.write("="*80 + "\n\n")
        f.write(f"Model: HMF_MultiDataset (Multi-Stream Hierarchical Fusion)\n")
        f.write(f"Architecture: All streams use MobileNetV3 (576) + Mamba (D=160)\n")
        f.write(f"  - Local Stream: MobileNetV3 + Mamba (160)\n")
        f.write(f"  - Context Stream: MobileNetV3 + Mamba (160)\n")
        f.write(f"  - Global Stream: MobileNetV3 + Mamba (160)\n")
        f.write(f"  - Audio Stream: MobileNetV3 + Mamba (160)\n")
        f.write(f"  - Fusion: Concat (640) + Mamba → MLP\n")
        f.write(f"Trained on: AVLips + DeepSpeak + LipSyncTIMIT (Multi-Dataset)\n")
    ...
    print("="*80)
    print(f"\n📝 Summary:")
    print(f"   Model: Multi-Dataset (4x MobileNetV3 + Mamba)")
    print(f"   Training: AVLips + DeepSpeak + LipSyncTIMIT")
    print(f"   Test: KoDF")
