...
"""
ABLATION STUDY - V1_HMF_FacialAudioCNN: Hierarchical Mamba Fusion (Smarter CNNs)

[FACIAL-CNN CHANGE]: This script uses domain-specific backbones.
It uses a "heavy" VGGFace2-ResNet50 for the high-detail Local stream,
and a "heavy" ResNet-18 for the audio stream.

Visual (Local):   VGGFace2-ResNet50 (2048) → Mamba (T=16) ↘
Visual (Context): MobileNetV3 (576)        → Mamba (T=16) → Concat (T=16) → Fusion Mamba → MLP
Visual (Global):  MobileNetV3 (576)        → Mamba (T=16) ↗
Audio:            ResNet-18 (512)          → Mamba (T=16) ↗
...
# [FACIAL-CNN CHANGE]: Import InceptionResnetV1 from facenet-pytorch
# You must install this: pip install facenet-pytorch
try:
    from facenet_pytorch import InceptionResnetV1
except ImportError:
    print("="*80)
    print("ERROR: `facenet-pytorch` not found.")
    print("Please install it: pip install facenet-pytorch")
    print("This is required for the VGGFace2-pretrained backbone.")
    print("="*80)
    exit()
...
class Config:
    def __init__(self):
        ...
        self.model_save_dir = "/home/affshafee/T2430421/pipelines/novel_fusion/HMF_facial_audio_cnn"
        ...
        # [FACIAL-CNN CHANGE]: We now have three CNN types
        # Heavy FACE backbone (facenet-pytorch, 512 features)
        self.cnn_feature_dim_face = 512
        # Light IMAGE backbone (MobileNetV3-Small, 576 features)
        self.cnn_feature_dim_light_img = 576  
        # Heavy AUDIO backbone (ResNet-18, 512 features)
        self.cnn_feature_dim_heavy_aud = 512
        ...
        # --- Training Parameters ---
        self.batch_size = 32 # Keep batch size low for heavy models
...
print(f"⚡ ARCHITECTURE: [FACIAL-AUDIO-CNN] Hierarchical Mamba Fusion (T={config.vis_num_frames})")
print(f"   Visual (Local):   VGGFace2-ResNet (Heavy)")
print(f"   Visual (Context): MobileNetV3-Small (Light)")
print(f"   Visual (Global):  MobileNetV3-Small (Light)")
print(f"   Audio Stream:     ResNet-18 (Heavy)")
...
class VisualStream_Light_MobileNetV3(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.proj = nn.Linear(config.cnn_feature_dim_light_img, config.vis_mamba_d_model)
...
#[FACIAL-CNN CHANGE]: NEW Class for the "Heavy" FACE backbone
class VisualStream_Heavy_FaceNet(nn.Module):
    """Heavyweight VGGFace2-ResNet stream for Local/Mouth view."""
    def __init__(self, config):
        super().__init__()
        # Load VGGFace2-pretrained ResNet
        # It's technically an "Inception ResNet", which is even better.
        self.cnn_features = InceptionResnetV1(pretrained='vggface2')
        # We'll use all layers *except* the final classification layer (logits)
        # We'll get the features from the `avgpool_1a` layer
        
        # Facenet-pytorch model is a bit different, we'll redefine the forward
        # to stop before the final `logits` layer
        
        print("---")
        print("✅ Loaded InceptionResnetV1 (pretrained on VGGFace2) for Local Stream.")
        print("---")
        
        # Project from heavy dim -> mamba dim
        self.proj = nn.Linear(config.cnn_feature_dim_face, config.vis_mamba_d_model)
        self.proj_dropout = nn.Dropout(0.3)
        self.mamba = Mamba(d_model=config.vis_mamba_d_model, d_state=16, d_conv=4, expand=2)
        self.mamba_dropout = nn.Dropout(0.2)
    ...
    def forward(self, x):
        ...
        # [FACIAL-CNN CHANGE]: Manually perform the forward pass of InceptionResnetV1
        # to get the 512-dim embedding *before* the classification head.
        x = self.cnn_features.conv2d_1a(x)
        x = self.cnn_features.conv2d_2a(x)
        x = self.cnn_features.conv2d_2b(x)
        x = self.cnn_features.maxpool_3a(x)
        x = self.cnn_features.conv2d_3b(x)
        x = self.cnn_features.conv2d_4a(x)
        x = self.cnn_features.conv2d_4b(x)
        x = self.cnn_features.repeat_1(x)
        x = self.cnn_features.mixed_6a(x)
        x = self.cnn_features.repeat_2(x)
        x = self.cnn_features.mixed_7a(x)
        x = self.cnn_features.repeat_3(x)
        x = self.cnn_features.block8(x)
        x = self.cnn_features.avgpool_1a(x)
        x = self.cnn_features.dropout(x)
        features_512d = x.view(x.size(0), -1) # Shape: (B*T, 512)
        
        features = features_512d.view(b, t, -1) # Shape: (B, T, 512)
...
#[FACIAL-CNN CHANGE]: NEW Class for the "Heavy" AUDIO backbone
class AudioStream_Heavy_ResNet18(nn.Module):
    """Heavyweight ResNet-18 stream for Audio (Melspectrograms)."""
    def __init__(self, config):
        super().__init__()
        # Load ResNet-18
        resnet = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
        
        # [FACIAL-CNN CHANGE]: Modify the first conv layer to accept 1 channel (mel)
        # instead of 3 (RGB). We'll average the weights.
        ...
        print("---")
        print("✅ Loaded ResNet-18 (1-channel input) for Audio Stream.")
        print("---")
        
        # Project from heavy dim -> mamba dim
        self.proj = nn.Linear(config.cnn_feature_dim_heavy_aud, config.aud_mamba_d_model)
        self.proj_dropout = nn.Dropout(0.3)
...
#[FACIAL-CNN CHANGE]: This model instantiates the new domain-specific streams.
...
def main():
    ...
    print("\n" + "="*80 + "\nSTEP 4: BUILDING [FACIAL-AUDIO-CNN] HIERARCHICAL FUSION MODEL\n" + "="*80)
    ...
    print(f"📊 [FACIAL-AUDIO-CNN] HierarchicalFusionModel Statistics:")
    print(f"   Total Parameters: {total_params:,}")
    print(f"   Trainable Parameters: {trainable_params:,}")
    print(f"   Model Size: {model_size_mb:.2f} MB")
    print(f"   Parameters (Millions): {total_params/1e6:.3f}M")
    print(f"   (Note: Size reflects VGGFace2-ResNet + ResNet18 backbones)")

    print("\n" + "="*80 + "\nSTEP 5: TRAINING [FACIAL-AUDIO-CNN] HIERARCHICAL FUSION MODEL\n" + "="*80)
    ...
    model_path = os.path.join(config.model_save_dir, 'v1_hmf_facial_audio_cnn_best.pth')
    ...
    print(f"\n" + "="*80)
    print(f"📈 V1_HMF_FacialAudioCNN: HIERARCHICAL FUSION (DOMAIN-CNN) - FINAL RESULTS")
    print(f"="*80)
    ...
    plt.title('V1_HMF_FacialAudioCNN (Hierarchical Fusion): Training and Validation Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(config.model_save_dir, 'v1_hmf_facial_audio_cnn_loss_curve.png'), dpi=150, bbox_inches='tight')
    plt.show()

    print("\n✅ V1_HMF_FacialAudioCNN (Hierarchical Fusion) ablation study completed!")
    print("📝 Record these metrics for your Excel comparison:")
    print(f"   - Variant: V1_HMF_FacialAudioCNN")
