...
"""
CROSS-DATASET EVALUATION: HMF_Superior (Multi-Stream) (AVLips) → KoDF
This script evaluates the Multi-Stream Hierarchical Mamba Fusion model trained on AVLips against KoDF.
Architecture: 3 Visual Streams (Local, Context, Global) + 1 Audio Stream → Fusion Mamba → MLP
"""
...
class Config:
    def __init__(self):
        ...
        self.model_path = "/home/affshafee/T2430421/pipelines/novel_fusion/HMF_superior/v1_hmf_multistream_best.pth"
        self.output_dir = "/home/affshafee/T2430421/pipelines/HMFs_AVL_to_KoDF"
        ...
        self.vis_num_frames = 16  # Multi-stream HMF uses T=16
        ...
        self.aud_chunk_duration = 5.0 / 16.0  # Total 5 seconds
        self.aud_n_mels = 128
...
print(f"   3 Visual Streams (Local + Context + Global) + 1 Audio Stream")
...
# --- 6. MULTI-STREAM MODEL ARCHITECTURE ---
...
class HierarchicalFusionModel(nn.Module):
    ...
    3 visual streams (Local, Context, Global) + 1 audio stream → Fusion Mamba → MLP
    """
    def __init__(self, config):
        super().__init__()
        # Instantiate all 3 visual streams
        ...
        # Define the final "Fusion Mamba Block"
        # Input dimension is sum of ALL stream dimensions (3 * vis + 1 * aud)
    ...
    def forward(self, visual_input_local, visual_input_context, visual_input_global, audio_input):
        ...
        fused_seq = torch.cat((local_seq, context_seq, global_seq, audio_seq), dim=2)  # (B, T, 3*D_vis + D_aud)

        # 3. Feed the fused sequence into the final Mamba block
        fusion_mamba_out = self.fusion_mamba(fused_seq)
        fusion_mamba_out = self.fusion_mamba_dropout(fusion_mamba_out)

        # 4. Take the final output vector
...
def plot_confusion_matrix(cm, output_path):
    ...
    plt.title('Confusion Matrix: HMF_Superior (Multi-Stream) (AVLips) → KoDF')
...
def main():
    print("\n" + "="*80)
    print("CROSS-DATASET EVALUATION: HMF_Superior (Multi-Stream) (AVLips) → KoDF")
    print("="*80)
    ...
    # Step 3: Load model
    print("\n" + "="*80)
    print("LOADING MULTI-STREAM HMF MODEL")
    print("="*80)
    ...
    print("="*80)
    print(f"\n🔥 Model: HMF_Superior (Multi-Stream Hierarchical Fusion)")
    print(f"   Architecture: 3 Visual Streams + 1 Audio Stream → Fusion Mamba")
    ...
    cm_path = os.path.join(config.output_dir, 'confusion_matrix_hmfs_kodf.png')
    plot_confusion_matrix(metrics['confusion_matrix'], cm_path)

    # Step 8: Save metrics to file
    results_path = os.path.join(config.output_dir, 'hmfs_evaluation_results.txt')
    with open(results_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("CROSS-DATASET EVALUATION: HMF_Superior (Multi-Stream) (AVLips) → KoDF\n")
        f.write("="*80 + "\n\n")
        f.write(f"Model: HMF_Superior (Multi-Stream Hierarchical Mamba Fusion)\n")
        f.write(f"Architecture: 3 Visual Streams (Local+Context+Global) + 1 Audio Stream\n")
    ...
    print("="*80)
    print(f"\n📝 Summary:")
    print(f"   Model: Multi-Stream HMF (3 Visual + 1 Audio)")
