...
"""
CROSS-DATASET EVALUATION: HMF_FacialAudioCNN (AVLips) → KoDF
Evaluates the Facial+Audio CNN Hierarchical Fusion model trained on AVLips against KoDF.
Architecture: VGGFace2-ResNet (Local) + MobileNetV3 (Context/Global) + ResNet18 (Audio)
"""
...
class Config:
    def __init__(self):
        ...
        self.model_path = "/home/affshafee/T2430421/pipelines/novel_fusion/HMF_facial_audio_cnn/v1_hmf_facial_audio_cnn_best.pth"
        self.output_dir = "/home/affshafee/T2430421/pipelines/HMF_fa_AVL_to_KoDF"
        ...
        # Local stream uses VGGFace2-ResNet (InceptionResnetV1: 512 features)
        self.vis_local_cnn_feature_dim = 512
        self.vis_local_mamba_d_model = 256

        # Context & Global use MobileNetV3-Small
        self.vis_context_cnn_feature_dim = 576
        self.vis_context_mamba_d_model = 160
        self.vis_global_cnn_feature_dim = 576
        self.vis_global_mamba_d_model = 160

        # --- Audio Stream (ResNet-18) ---
        self.aud_sample_rate = 16000
        self.aud_num_chunks = 16
        self.aud_chunk_duration = 5.0 / 16.0
        self.aud_n_mels = 128
        self.aud_cnn_feature_dim = 512  # ResNet-18 output
        self.aud_mamba_d_model = 256

        # --- Evaluation Parameters ---
        self.batch_size = 16  # Reduced for VGGFace2-ResNet
        self.num_workers = 8
...
print(f"⚡ ARCHITECTURE: Facial+Audio CNN Hierarchical Fusion (T={config.vis_num_frames})")
print(f"   Local: VGGFace2-ResNet (512) + Mamba (256)")
print(f"   Context/Global: MobileNetV3 (576) + Mamba (160)")
print(f"   Audio: ResNet-18 (512) + Mamba (256)")
...
class VisualStream_Heavy_FaceNet(nn.Module):
    """Heavyweight VGGFace2-ResNet stream for Local/Mouth view."""
    def __init__(self, config):
        super().__init__()
        self.cnn_features = InceptionResnetV1(pretrained='vggface2')
    ...
    def forward(self, x):
        ...
        features_512d = self.cnn_features(x)
        features = features_512d.view(b, t, -1)
...
class VisualStream_Light_MobileNet(nn.Module):
...
class AudioStream_Heavy_ResNet18(nn.Module):
    """Heavyweight ResNet-18 for audio stream."""
    def __init__(self, config):
        super().__init__()
...
class HierarchicalFusionModel(nn.Module):
    """Hierarchical Fusion Model with domain-specific CNNs."""
    def __init__(self, config):
        super().__init__()
        self.visual_stream_local = VisualStream_Heavy_FaceNet(config)
        self.visual_stream_context = VisualStream_Light_MobileNet(config, stream_type='context')
        self.visual_stream_global = VisualStream_Light_MobileNet(config, stream_type='global')
        self.audio_stream = AudioStream_Heavy_ResNet18(config)
...
def plot_confusion_matrix(cm, output_path):
    ...
    plt.title('Confusion Matrix: HMF_FacialAudioCNN (AVLips) → KoDF')
...
def main():
    print("\n" + "="*80)
    print("CROSS-DATASET EVALUATION: HMF_FacialAudioCNN (AVLips) → KoDF")
    print("="*80)
    ...
    print(f"⚠️  Batch size: {config.batch_size} (reduced for VGGFace2-ResNet)")

    # Step 3: Load model
    print("\n" + "="*80)
    print("LOADING FACIAL+AUDIO CNN MODEL")
    print("="*80)
    ...
    print("="*80)
    print(f"\n🔥 Model: HMF_FacialAudioCNN (Domain-Specific Backbones)")
    print(f"   Local: VGGFace2-ResNet + Mamba (256)")
    print(f"   Context/Global: MobileNetV3 + Mamba (160)")
    print(f"   Audio: ResNet-18 + Mamba (256)")
    ...
    cm_path = os.path.join(config.output_dir, 'confusion_matrix_hmf_fa_kodf.png')
    plot_confusion_matrix(metrics['confusion_matrix'], cm_path)

    # Step 8: Save metrics to file
    results_path = os.path.join(config.output_dir, 'hmf_fa_evaluation_results.txt')
    with open(results_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("CROSS-DATASET EVALUATION: HMF_FacialAudioCNN (AVLips) → KoDF\n")
        f.write("="*80 + "\n\n")
        f.write(f"Model: HMF_FacialAudioCNN (Domain-Specific Backbones)\n")
        f.write(f"Architecture:\n")
        f.write(f"  - Local Stream: VGGFace2-ResNet (512) + Mamba (256)\n")
        f.write(f"  - Context Stream: MobileNetV3 (576) + Mamba (160)\n")
        f.write(f"  - Global Stream: MobileNetV3 (576) + Mamba (160)\n")
        f.write(f"  - Audio Stream: ResNet-18 (512) + Mamba (256)\n")
        f.write(f"  - Fusion: Concat (832) + Mamba → MLP\n")
    ...
    print("="*80)
    print(f"\n📝 Summary:")
    print(f"   Model: Facial+Audio CNN (VGGFace2 + ResNet18)")
