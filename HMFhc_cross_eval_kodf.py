...
"""
CROSS-DATASET EVALUATION: HMF_Hybrid_CNN (AVLips) → KoDF

This script evaluates the Hybrid CNN Hierarchical Mamba Fusion model trained on AVLips against KoDF.
Architecture: 3 Visual Streams + 1 Audio Stream → Fusion Mamba → MLP
  - Visual (Local/Mouth): ResNet-50 (Heavy) → Mamba (T=16)
  - Visual (Context): MobileNetV3-Small (Light) → Mamba (T=16)
  - Visual (Global): MobileNetV3-Small (Light) → Mamba (T=16)
  - Audio: MobileNetV3-Small (Light) → Mamba (T=16)
"""
...
class Config:
    def __init__(self):
        ...
        self.model_path = "/home/affshafee/T2430421/pipelines/novel_fusion/HMF_hybrid_cnn/v1_hmf_hybrid_cnn_best.pth"
        self.output_dir = "/home/affshafee/T2430421/pipelines/HMFhc_AVL_to_KoDF"
...
#[HYBRID-CNN CHANGE]: Lightweight MobileNetV3-Small stream
...
#[HYBRID-CNN CHANGE]: Heavyweight ResNet-50 stream for Local/Mouth
...
#[HYBRID-CNN CHANGE]: Lightweight audio stream
...
#[HYBRID-CNN CHANGE]: This model now instantiates the hybrid streams.
...
def plot_confusion_matrix(cm, output_path):
    ...
    plt.title('Confusion Matrix: HMF_Hybrid_CNN (AVLips) → KoDF')
...
def main():
    print("\n" + "="*80)
    print("CROSS-DATASET EVALUATION: HMF_Hybrid_CNN (AVLips) → KoDF")
    print("="*80)
    ...
    # Step 3: Load model
    print("\n" + "="*80)
    print("LOADING HYBRID-CNN HMF MODEL")
    print("="*80)
    ...
    print("="*80)
    print(f"\n🔥 Model: HMF_Hybrid_CNN (Hierarchical Fusion with Hybrid Backbone)")
    ...
    cm_path = os.path.join(config.output_dir, 'confusion_matrix_hmfhc_kodf.png')
    plot_confusion_matrix(metrics['confusion_matrix'], cm_path)

    # Step 8: Save metrics to file
    results_path = os.path.join(config.output_dir, 'hmfhc_evaluation_results.txt')
    with open(results_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("CROSS-DATASET EVALUATION: HMF_Hybrid_CNN (AVLips) → KoDF\n")
        f.write("="*80 + "\n\n")
        f.write(f"Model: HMF_Hybrid_CNN (Hierarchical Mamba Fusion with Hybrid CNN)\n")
        f.write(f"Architecture: ResNet-50 (Local) + MobileNetV3 (Context/Global/Audio)\n")
    ...
    print("="*80)
    print(f"\n📝 Summary:")
    print(f"   Model: Hybrid-CNN HMF (ResNet-50 Local + MobileNetV3 Others)")
