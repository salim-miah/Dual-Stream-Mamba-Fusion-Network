...
"""
ABLATION STUDY - V1_HMF_MultiStream: Hierarchical Mamba Fusion (Multi-Stream)

[cite_start]This script implements the "Hierarchical Mamba Fusion" architecture[cite: 7]
with a multi-stream visual frontend (Local, Context, Global) as suggested.

It uses a fourth Mamba block to explicitly model the temporal relationship
[cite_start]between all three visual streams and the audio stream[cite: 8].

Visual (Local):   MobileNetV3-Small (576) → Mamba (T=16) ↘
Visual (Context): MobileNetV3-Small (576) → Mamba (T=16) → Concat (T=16) → Fusion Mamba → MLP
Visual (Global):  MobileNetV3-Small (576) → Mamba (T=16) ↗
Audio:            MobileNetV3-Small (576) → Mamba (T=16) ↗
...
class Config:
    def __init__(self):
        ...
        self.model_save_dir = "/home/affshafee/T2430421/pipelines/novel_fusion/HMF_superior"
        ...
        # --- Training Parameters ---
        self.batch_size = 64 # [MULTI-STREAM CHANGE]: May need to lower this if VRAM is an issue
...
print(f"   Visual Streams (x3): MobileNetV3-Small (576 features)")
print(f"   Audio Stream (x1): MobileNetV3-Small (576 features)")
...
#[MULTI-STREAM CHANGE]: This function now extracts and returns THREE visual crops.
def process_visual_stream(video_path: str, config: Config):
    ...
    # [MULTI-STREAM CHANGE]: Create lists for each stream
    local_frames_list = []
    context_frames_list = []
    global_frames_list = []
    ...
    for i in frame_indices:
        ...
        if len(faces) > 0:
            ...
            # [MULTI-STREAM CHANGE]: Check all crops are valid before resizing
    ...
    # [MULTI-STREAM CHANGE]: Ensure all streams have the correct number of frames
...
class DualStreamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            # [MULTI-STREAM CHANGE]: Get tuple of three visual streams
            ...
            # [MULTI-STREAM CHANGE]: Transpose all three streams
            ...
            # [MULTI-STREAM CHANGE]: Return all streams
...
class RAMCachedDataset(Dataset):
    ...
    #[MULTI-STREAM CHANGE]: Helper function to apply transform to a single stream
    def _apply_transform(self, frames_np_array):
        if self.transform:
            augmented_frames = []
            for frame_np in frames_np_array:
                # frame_np is (C, H, W), transpose to (H, W, C) for PIL
    ...
    def __getitem__(self, idx):
        # [MULTI-STREAM CHANGE]: Unpack all cached streams
        local_frames_np, context_frames_np, global_frames_np, audio_tensor = self.data[idx]
        label = self.labels[idx]

        # [MULTI-STREAM CHANGE]: Apply transform to each visual stream independently
        ...
        # [MULTI-STREAM CHANGE]: Return tuple of all data
...
#[MULTI-STREAM CHANGE]: This model is heavily modified.
class HierarchicalFusionModel(nn.Module):
    """
    [MULTI-STREAM CHANGE]: This is the new multi-stream fusion model.
    It has 3 visual streams (Local, Context, Global) and 1 audio stream.
    A final Mamba block models the temporal relationship between all 4 streams.
    """
    def __init__(self, config):
        super().__init__()
        # [MULTI-STREAM CHANGE]: Instantiate all 3 visual streams
        ...
        # Instantiate 1 audio stream
        self.audio_stream = AudioStream_MobileNetV3Small(config)

        # [MULTI-STREAM CHANGE]: Define the final "Fusion Mamba Block"
        # Its input dimension is the sum of ALL stream dimensions (3 * vis + 1 * aud)
    ...
    #[MULTI-STREAM CHANGE]: Forward pass now takes 4 inputs
    def forward(self, visual_input_local, visual_input_context, visual_input_global, audio_input):
        ...
        # Shape becomes (B, T, 3*D_vis + D_aud)
...
#[MULTI-STREAM CHANGE]: Updated data unpacking in train/val loops
def train_one_epoch(model, loader, optimizer, criterion, scaler, config):
    ...
    for i, ((visual_inputs, audio_data), labels) in enumerate(pbar):
        # [MULTI-STREAM CHANGE]: Unpack all 3 visual streams
        ...
        with autocast():
            # [MULTI-STREAM CHANGE]: Pass all 4 inputs to the model
...
def main():
    ...
    def cache_data(files, labels, desc):
        ...
        # [MULTI-STREAM CHANGE]: Lower num_workers if RAM is an issue during caching
        ...
        for data, batch_labels in tqdm(loader, desc=f"Caching {desc}"):
            if data is not None:
                # [MULTI-STREAM CHANGE]: Unpack visual tuple and audio
                ...
                for i in range(local_batch.shape[0]):
                    # [MULTI-STREAM CHANGE]: Cache all streams
                    cached_data.append((
                        local_batch[i].numpy(), 
    ...
    # [MULTI-STREAM CHANGE]: Lower num_workers if CPU/RAM is an issue
    ...
    print(f"   (Note: Params ~3x V1_HMF due to 3 visual streams)")
    ...
    model_path = os.path.join(config.model_save_dir, 'v1_hmf_multistream_best.pth')
    ...
    with torch.no_grad():
        #[MULTI-STREAM CHANGE]: Updated data unpacking
    ...
    print(f"\n" + "="*80)
    print(f"📈 V1_HMF_MultiStream: HIERARCHICAL FUSION (3-STREAM) - FINAL RESULTS")
    print(f"="*80)
    ...
    plt.title('V1_HMF_MultiStream (Hierarchical Fusion): Training and Validation Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(config.model_save_dir, 'v1_hmf_multistream_loss_curve.png'), dpi=150, bbox_inches='tight')
    plt.show()

    print("\n✅ V1_HMF_MultiStream (Hierarchical Fusion) ablation study completed!")
    print("📝 Record these metrics for your Excel comparison:")
    print(f"   - Variant: V1_HMF_MultiStream")
