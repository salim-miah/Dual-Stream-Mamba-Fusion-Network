...
"""
CROSS-DATASET EVALUATION: HMF_HybridCNN (AVLips) → KoDF
This script evaluates the Hybrid CNN Hierarchical Mamba Fusion model trained on AVLips against KoDF.
Architecture: ResNet-50 (Local) + MobileNetV3 (Context/Global/Audio) → Fusion Mamba → MLP
"""
...
class Config:
    def __init__(self):
        ...
        self.model_path = "/home/affshafee/T2430421/pipelines/novel_fusion/HMF_hybrid_cnn/v1_hmf_hybridcnn_best.pth"
        self.output_dir = "/home/affshafee/T2430421/pipelines/HMF_hybrid_AVL_to_KoDF"
        ...
        # Local stream uses ResNet-50
        self.vis_local_cnn_feature_dim = 2048  # ResNet-50 output
        self.vis_local_mamba_d_model = 256     # Larger for ResNet-50

        # Context & Global use MobileNetV3-Small
        self.vis_context_cnn_feature_dim = 576  # MobileNetV3-Small
        self.vis_context_mamba_d_model = 160
        self.vis_global_cnn_feature_dim = 576   # MobileNetV3-Small
        self.vis_global_mamba_d_model = 160
        ...
        self.batch_size = 16  # Reduced due to ResNet-50 memory requirements
        self.num_workers = 8
...
print(f"   Local: ResNet-50 (2048) + Mamba (D=256)")
print(f"   Context/Global: MobileNetV3 (576) + Mamba (D=160)")
print(f"   Audio: MobileNetV3 (576) + Mamba (D=160)")
...
def process_visual_stream(video_path: str, config: Config):
    """Extract and process THREE visual streams (Local, Context, Global) from video."""
    ...
    # Multi-stream: Create lists for each stream
    local_frames_list = []
    context_frames_list = []
    global_frames_list = []
    ...
    # Ensure all streams have correct number of frames
...
class KoDFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            # Process visual streams (returns tuple of 3 streams)
            ...
            if self.transform:
                # Transform each stream independently
                local_frames = []
                context_frames = []
                global_frames = []

                for i in range(len(local_frames_hwc)):
                    local_frames.append(self.transform(local_frames_hwc[i]))
                    context_frames.append(self.transform(context_frames_hwc[i]))
                    global_frames.append(self.transform(global_frames_hwc[i]))

                local_tensor = torch.stack(local_frames)
                context_tensor = torch.stack(context_frames)
                global_tensor = torch.stack(global_frames)
            else:
                local_tensor = torch.from_numpy(local_frames_hwc.transpose(0, 3, 1, 2)).float()
                context_tensor = torch.from_numpy(context_frames_hwc.transpose(0, 3, 1, 2)).float()
                global_tensor = torch.from_numpy(global_frames_hwc.transpose(0, 3, 1, 2)).float()

            audio_tensor = audio_mels.unsqueeze(1)

            # Return tuple of visual streams and audio
            visual_data_tuple = (local_tensor, context_tensor, global_tensor)

            return (visual_data_tuple, audio_tensor), torch.tensor(label, dtype=torch.float32)
        except Exception as e:
            return None

# --- 6. HYBRID CNN MODEL ARCHITECTURE ---
class VisualStream_ResNet50(nn.Module):
    """Heavy ResNet-50 for high-detail Local stream."""
    def __init__(self, config):
        super().__init__()
        resnet = models.resnet50(weights=models.ResNet50_Weights.DEFAULT)
        # Remove final FC layer
        self.cnn_features = nn.Sequential(*list(resnet.children())[:-1])
        self.proj = nn.Linear(config.vis_local_cnn_feature_dim, config.vis_local_mamba_d_model)
        self.proj_dropout = nn.Dropout(0.3)
        self.mamba = Mamba(d_model=config.vis_local_mamba_d_model, d_state=16, d_conv=4, expand=2)
    ...
    def forward(self, x):
        ...
        return temporal_out  # Return full sequence (B, T, D)

class VisualStream_MobileNetV3Small(nn.Module):
    """Lightweight MobileNetV3-Small for Context/Global streams."""
    def __init__(self, config, stream_type='context'):
        super().__init__()
        mobilenet = models.mobilenet_v3_small(weights=models.MobileNet_V3_Small_Weights.DEFAULT)
        self.cnn_features = mobilenet.features
        self.avgpool = mobilenet.avgpool

        # Use different d_model based on stream type
        if stream_type == 'context':
            cnn_dim = config.vis_context_cnn_feature_dim
            mamba_dim = config.vis_context_mamba_d_model
        else:  # global
            cnn_dim = config.vis_global_cnn_feature_dim
            mamba_dim = config.vis_global_mamba_d_model

        self.proj = nn.Linear(cnn_dim, mamba_dim)
        self.proj_dropout = nn.Dropout(0.3)
        self.mamba = Mamba(d_model=mamba_dim, d_state=16, d_conv=4, expand=2)
        self.mamba_dropout = nn.Dropout(0.2)
...
class AudioStream_MobileNetV3Small(nn.Module):
    """MobileNetV3-Small for audio feature extraction."""
    def __init__(self, config):
        super().__init__()
        mobilenet = models.mobilenet_v3_small(weights=models.MobileNet_V3_Small_Weights.DEFAULT)
        self.cnn_features = mobilenet.features
...
class HybridMultiCNNFusion(nn.Module):
    """
    Hybrid CNN Multi-Stream Hierarchical Fusion Model.
    Local: ResNet-50 → Mamba
    Context: MobileNetV3 → Mamba
    Global: MobileNetV3 → Mamba
    Audio: MobileNetV3 → Mamba
    All → Fusion Mamba → MLP
    """
    def __init__(self, config):
        super().__init__()
        # Local stream: ResNet-50 (heavy)
        self.visual_stream_local = VisualStream_ResNet50(config)

        # Context stream: MobileNetV3-Small (light)
        self.visual_stream_context = VisualStream_MobileNetV3Small(config, stream_type='context')

        # Global stream: MobileNetV3-Small (light)
        self.visual_stream_global = VisualStream_MobileNetV3Small(config, stream_type='global')

        # Audio stream: MobileNetV3-Small (light)
        self.audio_stream = AudioStream_MobileNetV3Small(config)

        # Fusion Mamba Block
        fusion_mamba_d_model = (config.vis_local_mamba_d_model + 
                               config.vis_context_mamba_d_model + 
                               config.vis_global_mamba_d_model + 
                               config.aud_mamba_d_model)
    ...
    def forward(self, visual_input_local, visual_input_context, visual_input_global, audio_input):
        # Get full sequences from all streams
        local_seq = self.visual_stream_local(visual_input_local)      # (B, T, 256)
        context_seq = self.visual_stream_context(visual_input_context) # (B, T, 160)
        global_seq = self.visual_stream_global(visual_input_global)    # (B, T, 160)
        audio_seq = self.audio_stream(audio_input)                     # (B, T, 160)

        # Concatenate all streams
        fused_seq = torch.cat((local_seq, context_seq, global_seq, audio_seq), dim=2)  # (B, T, 736)

        # Feed through fusion Mamba
...
def evaluate_model(model, dataloader, config):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            (visual_data_tuple, audio_data), labels = batch
            local_data, context_data, global_data = visual_data_tuple
...
def plot_confusion_matrix(cm, output_path):
    ...
    plt.title('Confusion Matrix: HMF_HybridCNN (AVLips) → KoDF')
...
def main():
    print("\n" + "="*80)
    print("CROSS-DATASET EVALUATION: HMF_HybridCNN (AVLips) → KoDF")
    print("="*80)
    ...
    print(f"⚠️  Batch size reduced to {config.batch_size} due to ResNet-50 memory requirements")

    # Step 3: Load model
    print("\n" + "="*80)
    print("LOADING HYBRID CNN MODEL")
    print("="*80)

    model = HybridMultiCNNFusion(config).to(config.device)
    ...
    print("="*80)
    print(f"\n🔥 Model: HMF_HybridCNN")
    print(f"   Architecture: ResNet-50 (Local) + MobileNetV3 (Context/Global/Audio)")
    print(f"🎯 Trained on: AVLips")
    ...
    cm_path = os.path.join(config.output_dir, 'confusion_matrix_hmf_hybrid_kodf.png')
    plot_confusion_matrix(metrics['confusion_matrix'], cm_path)

    # Step 8: Save metrics to file
    results_path = os.path.join(config.output_dir, 'hmf_hybrid_evaluation_results.txt')
    with open(results_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("CROSS-DATASET EVALUATION: HMF_HybridCNN (AVLips) → KoDF\n")
        f.write("="*80 + "\n\n")
        f.write(f"Model: HMF_HybridCNN (Hybrid CNN Hierarchical Fusion)\n")
        f.write(f"Architecture:\n")
        f.write(f"  - Local Stream: ResNet-50 (2048) + Mamba (256)\n")
        f.write(f"  - Context Stream: MobileNetV3 (576) + Mamba (160)\n")
        f.write(f"  - Global Stream: MobileNetV3 (576) + Mamba (160)\n")
        f.write(f"  - Audio Stream: MobileNetV3 (576) + Mamba (160)\n")
        f.write(f"  - Fusion: Concat (736) + Mamba → MLP\n")
        f.write(f"Trained on: AVLips\n")
    ...
    print("="*80)
    print(f"\n📝 Summary:")
    print(f"   Model: Hybrid CNN (ResNet-50 + MobileNetV3)")
    print(f"   Training: AVLips")
    print(f"   Test: KoDF")
